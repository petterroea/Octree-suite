...
__global__ void kernel_cudaRender(loadedOctreeType* in, int rootOffset, cudaSurfaceObject_t output, cudaSurfaceObject_t iterationOutput, int imgWidth, int imgHeight, glm::mat4x4* view, glm::mat4x4* projection, int flipFlag) {
...
void cudaRender(loadedOctreeType* in, int rootOffset, cudaSurfaceObject_t out, cudaSurfaceObject_t iterations, int imgWidth, int imgHeight, void* view, void* projection, int flipFlag) {
    std::cout << "Rendering on GPU, offset " << rootOffset << std::endl;
    CUDA_CATCH_ERROR

    // Copy the cuda data structure to memory
    // TODO don't do every frame
    loadedOctreeType* containerCuda;
    cudaMalloc(&containerCuda, sizeof(loadedOctreeType));
    CUDA_CATCH_ERROR
    cudaMemcpy(containerCuda, in, sizeof(loadedOctreeType), cudaMemcpyHostToDevice);
    CUDA_CATCH_ERROR

    //Calculate number of blocks we need to run
    int blockCount = (imgWidth*imgHeight)/THREADS_PER_BLOCK;
    kernel_cudaRender<<<blockCount, THREADS_PER_BLOCK>>>(containerCuda, rootOffset, out, iterations, imgWidth, imgHeight, (glm::mat4x4*)view, (glm::mat4x4*)projection, flipFlag);
    CUDA_CATCH_ERROR
