...
void OctreeRenderer::cleanupTextures() {
 ...
 */
void OctreeRenderer::updateTexture(int width, int height) {
...
void OctreeRenderer::setupRgbTexture(int width, int height) {
...
void OctreeRenderer::setupIterationTexture(int width, int height) {
...
void OctreeRenderer::mapGlToCuda(GLuint glTexture, cudaArray_t* cudaArray, cudaSurfaceObject_t* surfaceObject) {
    // Create CUDA mapping
...
void OctreeRenderer::render(loadedOctreeType* currentFrameset, int frameIndex, glm::mat4 view, glm::mat4 projection) {
    // Invoke CUDA
        // Upload matrixes
    ...
    ImGui::End();
    //Run CUDA
    cudaRender(currentFrameset, frameIndex, this->outputSurfObjRgb, this->outputSurfObjIterations, this->textureWidth, this->textureHeight, viewMatrixPtr, projectionMatrixPtr, flipFlag);
