...
#include "layeredOctreeContainerVector.h"
#include "layeredOctreeContainerStatic.h"
...
public:
    LayeredOctreeContainerCuda(LayeredOctreeContainerVector<T>& container);
    LayeredOctreeContainerCuda(LayeredOctreeContainerStatic<T>& container);
...
/*
 * Copy-constructors that copies data to CUDA memory
 */
template <typename T>
LayeredOctreeContainerCuda<T>::LayeredOctreeContainerCuda(LayeredOctreeContainerVector<T>& container) {
...
template <typename T>
LayeredOctreeContainerCuda<T>::LayeredOctreeContainerCuda(LayeredOctreeContainerStatic<T>& container) {
    for(int i = 0; i < OCTREE_MAX_DEPTH; i++) {
        int layerSize = container.getLayerSize(i);
        this->layerSizes[i] = layerSize;
        if(layerSize == 0) {
...
__device__ LayeredOctree<T>* LayeredOctreeContainerCuda<T>::getNode(int layer, int idx) {
    //if(layer > OCTREE_MAX_DEPTH) return 0;
    return &this->layersGpu[layer][idx];
}

template <typename T>
LayeredOctreeContainerCuda<T>::~LayeredOctreeContainerCuda() {
    std::cout << "Deconstructing layered cuda octree" << std::endl;
    CUDA_CATCH_ERROR
    for(int i = 0; i < OCTREE_MAX_DEPTH; i++) {
